...
fut2 <- fut2[ , -c(2:4)]
df_l <- df_l[ , c("subject_id", "dt_kcal", "dt_fibe", "age", "sex", "bmi_final")]
...
cols_to_replace <- c("dt_fibe_pk_tertile")
...
df_filtered <- df[!df$dt_fibe_pk_tertile %in% c("T2", "T3"), ]

# Print the result
print(df_filtered)

# drop factor levels 
df_filtered$dt_fibe_pk_tertile <- droplevels(df_filtered$dt_fibe_pk_tertile) # cheers
...
permanova <- adonis2(t(otu) ~ dt_fibe_pk_tertile,
               data = meta, permutations=99, method = "bray")

print(permanova) # no significant difference in microbiome by secretor status (P = 0.41)
...
dist <- vegdist(t(otu))
anova(betadisper(dist, meta$dt_fibe_pk_tertile))

permutest(betadisper(dist, meta$dt_fibe_pk_tertile), pairwise = TRUE)
# p = 0.81
...
ggsave(filename = '/Users/yasmine.bouzid/Documents/FUT2_analysis/plots/permanova_tertileFFQlow.jpg', plot = p, device = 'jpeg', dpi = 300, width = 10, height = 6, units = "in")
