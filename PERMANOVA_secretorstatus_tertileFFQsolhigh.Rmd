...
Purpose: determine associations between secretor status and microbiome composition. We are comparing non-secretors to secretor high fiber consumers (based on ASA24 fiber here)
...
fut2 <- fut2[ , -c(2:4)]
df_l <- df_l[ , c("subject_id", "dt_kcal", "dt_fiber_sol", "age", "sex", "bmi_final")]
...
cols_to_replace <- c("dt_fiber_sol_pk_tertile")
...
df_filtered <- df[!df$dt_fiber_sol_pk_tertile %in% c("T1", "T2"), ]

# Print the result
print(df_filtered)

# drop factor levels 
df_filtered$dt_fiber_sol_pk_tertile <- droplevels(df_filtered$dt_fiber_sol_pk_tertile) # cheers
...
permanova <- adonis2(t(otu) ~ dt_fiber_sol_pk_tertile,
               data = meta, permutations=99, method = "bray")

print(permanova) # no significant difference in microbiome by secretor status (P = 0.52)
...
dist <- vegdist(t(otu))
anova(betadisper(dist, meta$dt_fiber_sol_pk_tertile))

permutest(betadisper(dist, meta$dt_fiber_sol_pk_tertile), pairwise = TRUE)
# p = 0.82
...
p <- plot_landscape(pseq.rel, method = "NMDS", distance = "bray", col = "secr_status", legend = T, size = 3) + 
  labs(title = "Non-metric Multi-dimensional Scaling Plot of Microbial Composition \n by Non-secretors and Secretor FFQ High Soluble Fiber Consumers",  x = "PC1",
       y = "PC2") +
#  scale_shape_discrete(labels = c("Non-secretor", "Secretor")) +
  scale_colour_brewer(labels = c("Non-secretor", "Secretor: High Fiber"),
                      palette = "Dark2") +
  theme_bw() + 
  theme(
...
ggsave(filename = '/Users/yasmine.bouzid/Documents/FUT2_analysis/plots/permanova_tertileFFQsolhigh.jpg', plot = p, device = 'jpeg', dpi = 300, width = 10, height = 6, units = "in")

```

**Summary of Findings**: 

- No difference in microbiome between high fiber secretors and non-secretors
